...
int main(void) {
    ...
    char OutFileName[] = "AusgabePV_Rechner.txt";                                       //Name der Datei wird definiert und abgespeichert unter AusgabePV_Rechner.txt
    ...
    char monatsnamen[12][10] = { "Jan", "Feb", "Mar", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez" };  //Moantsnamen werden definiert 
...
//Globalstrahlung auf horizontalen Flächen in Hamburg
    ...
    for (int neigung = 0; neigung <= 2; ++neigung) {                                    //alle drei Neigunngswinkel werden abgerufen, 0°, 30°,60°
        ...
        printf("\n\nDas jaehrliche Mittel betraegt: %.2lf\n", mittelwertJahr);
        fill('_', 130);
        printf("\n");
    }
    ...
    printf("\n\n\nPV-Rechner fuer den PV-Ertrag von PV-Modulen in verschiedenen Städten.\nNeigungswinkel und Spitzenwerte werden ebenfalls mit einberechnet\n\n\n");            //Ausgabe an Nutzer, kurze Erklärung
    ...
    for (int monat = 0; monat < 12; ++monat) {                                                                      //nur 12 Monate, beginn bei 0, variable wird erhöht
        printf("   %s  |", monatsnamen[monat]);                                                                     //Monate werden geprintet
        fprintf(out, "   %s  |", monatsnamen[monat]);
    }
    printf("\n");                                                                                                   //absatz
    ...
    for (int monat = 0; monat < 12; ++monat) {                                                                      //Moante werden durchlaufen
        ...
        fprintf(out, "%4.2lf |", wert);
        printf(" %6.2lf |", wert);
    }


    printf("\n\n");                                                                                                 //Absätze
    fill('_', 125);                                                                                                 //aufrufen der fill funktion für eine einfachere Trennung von ausgaben an den nutzer
    printf("\n");                                                                                                   //Absatz
